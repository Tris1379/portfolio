    ...
    <section className="py-16 md:py-24">
      <div className="max-w-4xl mx-auto px-4 md:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {config.sections.arsenal.categories.map((category, i) => (
              ...
              <div className="glass p-6 h-full">
                ...
                <div className="flex flex-wrap gap-2">
                  {category.skills.map((skill) => (
                    <span
                      key={skill}
                      className="px-3 py-1 rounded-full text-sm border border-(--color-primary)/30 text-(--color-primary) transition-colors duration-200 hover:border-(--color-primary)/60 hover:text-(--color-primary)"
                    >
                      {skill}
                    </span>
                  ))}
